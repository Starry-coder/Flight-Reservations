...
bg= PhotoImage(file='editflight2bg.png')
...
for i in a:
    temp.append(i)
u.close()
fetchfidstr.append(temp[-2])
fidd=fetchfidstr[0][0]+fetchfidstr[0][1]+fetchfidstr[0][2]+fetchfidstr[0][3]
#-----------------------------------------------------------------------------------------------------------------------
...
my_conn.execute("select * from flights where f_id='"+fidd+"'")
ls=list(my_conn.fetchall()[0])
fid.insert(0,ls[0])
fid['state']= tk.DISABLED
fco.insert(0,ls[1])
fsd.insert(0,ls[2])
ddate=ls[2]
fsd.config(state= "disabled")
fed.insert(0,ls[5])
datee=ls[5]
fed.config(state= "disabled")
if ls[9] is None:
    frdentry.insert(0,'~ NULL ~')
    frdentry.config(state= "disabled")
else:
    frdentry.insert(0,ls[9])
    frdentry.config(state= "disabled")
    dater=ls[9]
fst.insert(0,ls[3])
frt.insert(0,ls[4])
...
def Editflight():
    if str(fid.get())=='' or str(fco.get())=='' or str(fsd.get())=='' or str(fed.get())=='' or str(fst.get())=='' or str(frt.get())=='' or str(f_o.get())=='' or str(f_d.get())=='' or str(f_p.get())=='':
        ...
        if len(fst.get())==8 and len(frt.get())==8:
            ...
            if frdentry.get()=='~ NULL ~':
                my_conn.execute("UPDATE flights SET f_company='"+fco.get()+"',f_startdate='"+ddate+"',f_enddate='"+datee+"',f_returndate=      NULL          ,f_starttime='"+fst.get()+"',f_reachtime='"+frt.get()+"',f_startloc='"+f_o.get()+"',f_endloc='"+f_d.get()+"',f_price='"+f_p.get()+"' WHERE f_id='"+fid.get()+"';")
            else:
                my_conn.execute("UPDATE flights SET f_company='"+fco.get()+"',f_startdate='"+ddate+"',f_enddate='"+datee+"',f_returndate='"+dater+"',f_starttime='"+fst.get()+"',f_reachtime='"+frt.get()+"',f_startloc='"+f_o.get()+"',f_endloc='"+f_d.get()+"',f_price='"+f_p.get()+"' WHERE f_id='"+fid.get()+"';")
            my_connect.commit()
            mbox.showinfo('Success', 'Flight updated successfully')
            eewin.destroy()
            import main
        else:
...
def insertedate():
    ...
    datee='20'+yearr+'-'+monthr+'-'+dater
    fed['state'] = tk.DISABLED
    return temp,u,a,i,temp02,datee
...
b1= tk.Button(eewin,image=updatebuttonimg,command=Editflight,bd=0,bg='#fee9d0').place(x=350,y=550)
